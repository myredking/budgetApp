...
- YouTube 기본 일일 쿼터 10,000 단위와 영상 업로드 1,600 단위를 로컬 상태에 기록합니다. 한도에 도달하면 다음 API 호출을 차단하며, 쿼터 날짜는 YouTube 기준 태평양 시간으로 계산합니다.
- 업로드 요청 전에 pending 상태를 남깁니다. 업로드 후 완료 기록이 끊긴 파일은 중복 업로드하지 않고 수동 확인을 요구합니다.
...
예약 실행에서는 `--report outputs/albums/automation-report.json`을 추가할 수 있습니다. 보고서에는 앨범별 `ready`/`review_required` 상태, 차단 코드, 패키지·영상 경로, YouTube 결과와 전체 요약이 기록됩니다. 보고서는 로컬 JSON으로만 작성되며 외부 분석 서비스나 추가 사용료가 없습니다.
